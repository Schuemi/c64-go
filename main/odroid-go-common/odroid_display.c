...
SemaphoreHandle_t c64_mutex = NULL;

void odroid_display_lock_c64_display()
{
    if (!c64_mutex)
    {
        c64_mutex = xSemaphoreCreateMutex();
        if (!c64_mutex) abort();
    }

    if (xSemaphoreTake(c64_mutex, portMAX_DELAY) != pdTRUE)
    {
...
void odroid_display_unlock_c64_display()
{
    if (!c64_mutex) abort();
    xSemaphoreGive(c64_mutex);
}
void odroid_display_stop_c64_display()
{
    odroid_display_lock_c64_display();
    stopDisplay = 1;
    odroid_display_unlock_c64_display();
...
void odroid_display_resume_c64_display()
{
    stopDisplay = 0;
}
