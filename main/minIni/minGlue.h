...
#define ini_read(buffer,size,file)    (_fgets((buffer),(size),*(file)) != NULL)
#define ini_write(buffer,file)        (_fputs((buffer),*(file)) >= 0)
#define ini_rename(source,dest)       (_rename((source), (dest)) == 0)
#define ini_remove(filename)          (_remove(filename) == 0)
